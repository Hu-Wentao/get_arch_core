...
extension FutureEitherX<L, R> on Future<Either<L, R>> {
  ...
  Future<L> asyncLeftOrNull([void Function(R r)? onRight]) =>
      this.then((value) => value.leftOrNull(onRight)!);

  Future<B> asyncFold<B>(B ifLeft(L l), B ifRight(R r)) =>
      this.then((value) => value.fold<B>(ifLeft, ifRight));

  ///
  /// 包装了已有的Map
  Future<Either<L, R2>> asyncMap<R2>(R2 map(R r)) =>
      this.then((value) => value.fold(left, (R r) => right(map(r))));
}

extension SafeFutureEitherX<L, R> on Future<Either<L, R>>? {
  ///
  /// 返回值为Failure或null
  Future<L>? asyncLeftOrNull([
    void Function(R r)? onRight,
  ]) =>
      this?.then((value) => value.leftOrNull(onRight)!);

  Future<B> asyncFold<B>(B ifLeft(L? l), B ifRight(R? r), {B onNull()?}) =>
      this?.then((value) =>
          value.fold<B>(ifLeft, ifRight) ?? (onNull!() ?? ifLeft(null))) ??
      (onNull as Future<B>? ?? ifLeft(null) as Future<B>);
